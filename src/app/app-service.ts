...
export class AppService {
  ...
  getRecipeList() {
   ...
   this.httpClient.get<{message: string, recipes: any}>('http://localhost:3000/api/getRecipes/' + this.curPage).subscribe(result => {
  ...
  getRecipe(recipeDataId: string) {
    ...
    this.httpClient.get<{message: string, recipe: any}>("http://localhost:3000/api/getRecipe/" + recipeDataId).subscribe(result => {
  ...
  searchRecipes(recipeName: string) {
    this.httpClient.get<{message: string, searchResults: any}>('http://localhost:3000/api/searchRecipes/' + recipeName).subscribe(result => {
  ...
  postRecipe(recipeDetails: Recipe): Promise<boolean> {
    return new Promise((resolve, reject) => {
      ...
      this.authRequest().then(authState => {
        if(authState) {
          this.httpClient.post<{message: string, recipe: any}>('http://localhost:3000/api/createRecipe', postData, { withCredentials: true }).subscribe(response => {
  ...
  getUsername(): Promise<string> {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.httpClient.get<{ message: string, username: string, userStatus: string }>('http://localhost:3000/auth/getSessionUsername', { withCredentials: true })
        .subscribe(
          response => {
  ...
  deleteRecipe(recipeId: string) {
    ...
    this.authRequest().then(authState => {
      if(authState) {
        this.httpClient.delete<{message: string}>('http://localhost:3000/api/deleteRecipe/' + recipeId, {withCredentials: true}).subscribe(response => {
          this.getRecipeList();
        });
      }
    });
  ...
  saveRecipe(recipeId: string) {
    ...
    this.authRequest().then(authState => {
      if(authState) {
        this.httpClient.post<{message: string}>('http://localhost:3000/auth/saveRecipe', postData, { withCredentials: true }).subscribe(response => {
  ...
  removeSavedRecipe(recipeId: string) {
    ...
    this.authRequest().then(authState => {
      if(authState) {
        this.httpClient.post<{message: string}>('http://localhost:3000/auth/removeSavedRecipe', postData, { withCredentials: true }).subscribe(response => {
  ...
  checkRecipeSavedStatus(recipeId: string): Promise<boolean> {
    return new Promise<boolean>((resolve, reject) => {
      ...
      this.httpClient.post<{message: string}>('http://localhost:3000/auth/checkRecipeSavedStatus', postData, { withCredentials: true }).subscribe(response => {
        resolve(true);
      },
      error => {
        resolve(false);
  ...
  authRequest(): Promise<boolean> {
    return new Promise((resolve, reject) => {
      ...
      this.httpClient.get<{message: string}>('http://localhost:3000/auth/authRequest', { withCredentials: true }).subscribe(response => {
        resolve(true);
      },
      error => {
        console.error('Error occurred during HTTP request:', error);
